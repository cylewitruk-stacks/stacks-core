...
#[test]
/// Transactions with future nonces are held while the account's next nonce is
/// missing, and filling the gap releases the whole chain in nonce order.
///
/// This preserves the mempool behavior formerly covered by the Mocknet
/// `mine_transactions_out_of_order` integration test.
fn test_iterate_candidates_nonce_gap_hold_and_release() {
    ...
    assert!(
        considered.is_empty(),
        "future nonce transactions were considered before nonce 0: {considered:?}"
    );

    submit(&mut chainstate, &mut mempool, 0);
    assert_eq!(
        walk(&mut chainstate, &mut mempool, &mut tx_events),
        vec![0, 1, 2, 3]
    );
}
