...
use curve25519_dalek::scalar::{Scalar as ed25519_Scalar, clamp_integer};
use sha2::{Digest, Sha512};

fn hex_bytes(value: &str) -> Result<Vec<u8>, const_hex::FromHexError> {
    const_hex::decode(value)
}

fn to_hex(value: &[u8]) -> String {
    const_hex::encode(value)
}
    ...
    pub fn from_slice(bytes: &[u8]) -> Option<VRFProof> {
        match bytes.len() {
            80 => {
                ...
                let gamma = CompressedEdwardsY::from_slice(&bytes[0..32])
                    .ok()
...
impl VRF {
    ...
    pub fn prove(secret: &VRFPrivateKey, alpha: &[u8]) -> Option<VRFProof> {
        ...
        VRFProof::new(Gamma_point, c_scalar, s_scalar).ok()
    }
