...
use stacks_primitives::address::{AddressError, AddressHashMode, StacksAddress};
    ...
    Network: AddressNetwork,
{
    ...
    ) -> Result<StacksNetworkAddress<Network>, AddressError> {
    ...
    Network: AddressNetwork,
{
    ...
    fn try_from(address: StacksAddress) -> Result<Self, Self::Error> {
        let hash_mode = address_hash_mode_from_version_for_network::<Network>(address.version())
            .ok_or(AddressError::InvalidNetworkVersion(address.version()))?;
