...
run_node_suite attacknet-contract-node "${TEST_DIR}"/contracts/*.test.mjs
run_node_suite attacknet-equivalence-node "${TEST_DIR}"/equivalence/*.test.mjs
...
release_tests=("${ATTACKNET_DIR}"/release/*.test.mjs)
while IFS= read -r -d '' release_test; do
  release_tests+=("${release_test}")
done < <(find "${ATTACKNET_DIR}/release/amendments" -type f -name '*.test.mjs' -print0 2>/dev/null)
run_node_suite release-node "${release_tests[@]}"
