...
export const A7_SUMMARY_SCHEMA = 'stacks-attacknet-release-1-a7-evidence/v1';
export const A7_ARCHIVE_INDEX_SCHEMA = 'stacks-attacknet-evidence-archive-index/v1';
export const A7_ARTIFACTS = Object.freeze({
  candidateDiff: 'candidate.patch',
  deletedPaths: 'deleted-paths.json',
  verification: 'verification.json',
  attacknetCheck: 'attacknet-result.json',
  hacknetCheck: 'hacknet-result.json',
});
export const A7_ASSERTIONS = Object.freeze([
  'signed-a6-child-and-exact-candidate-diff',
  'complete-legacy-deletion-inventory',
  'active-qualification-consumers-have-current-owners',
  'golden-v1alpha1-equivalence-contracts-pass',
...
/** Validate the whole-product Attacknet result used by A7. */
export function validateA7AttacknetResult(value, candidateRevision) {
...
/** Validate the whole-product Hacknet result used by A7. */
export function validateA7HacknetResult(value, candidateRevision) {
  ...
  for (const required of ['go', 'helm']) {
    if (value.optionalChecks?.find(check => check.name === required)?.status !== 'passed') {
      fail(`A7 requires a passed Hacknet ${required} check`);
    }
  }
  return value;
}
...
function archiveIndex(candidateRevision, root) {
  ...
  visit(root);
  return {schema: A7_ARCHIVE_INDEX_SCHEMA, candidateRevision, entries};
}

/** Assemble a portable A7 archive from candidate-bound offline verification. */
export function assembleA7Evidence({
  candidateRevision,
  inputDirectory,
  outputDirectory,
  archiveLocation,
  ...
  const artifacts = {};
  for (const [key, filename] of Object.entries(A7_ARTIFACTS)) {
    ...
    if (key === 'candidateDiff') {
      ...
      if (key === 'verification') validateA7Verification(value, candidateRevision);
      else if (key === 'deletedPaths') validateA7DeletedPaths(value, candidateRevision);
      else if (key === 'attacknetCheck') validateA7AttacknetResult(value, candidateRevision);
      else validateA7HacknetResult(value, candidateRevision);
    }
  ...
  const archiveName = `release-1-a7-evidence-${candidateRevision.slice(0, 12)}.tar.gz`;
  ...
  const summary = {
    ...
    artifacts,
    assertions: A7_ASSERTIONS.map(id => ({id, status: 'passed'})),
  };
...
function main(arguments_) {
  ...
  process.stdout.write(`${JSON.stringify(assembleA7Evidence(options), null, 2)}\n`);
}
