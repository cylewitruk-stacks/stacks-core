...
const sourceEnvironment = 'ATTACKNET_A10_MATERIALIZED_SOURCE';
...
/** Materialize one immutable A10 candidate tree in an isolated shared clone. */
export function materializeQualifiedTree(repositoryRoot, qualifiedTree) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'attacknet-a10-source-'));
  ...
  try {
    ...
    return {sourceRoot, environment: {...environment, [sourceEnvironment]: sourceRoot}, cleanup: () => rmSync(root, {recursive: true, force: true})};
  } catch (error) {
...
/** Re-execute an A10 qualification entrypoint from its immutable source tree. */
...
/** Report whether this process already runs from the materialized A10 tree. */
