...
export const A10_BUILD_PURPOSES = Object.freeze([
  'topology-operator', 'run-operator', 'burnchain-clock', 'probe', 'io-pressure',
  'stacker', 'stacks-core',
]);

export const A10_INSTALLED_PURPOSES = Object.freeze([
  'topology-operator', 'run-operator', 'burnchain-clock', 'probe', 'io-pressure',
]);
...
function validateKindLoad(load, expectedRefs, expectedNodes = undefined) {
  ...
  if (nodes.size !== 3 || (expectedNodes && (nodes.size !== expectedNodes.size
  ...
  for (const image of load.images) {
    ...
    if (!expectedPairs.has(key) || observedPairs.has(key) || !immutableDigest.test(image.runtimeImageID ?? '')) {
...
/** Validate the exact image build and kind admission used by A10 qualification. */
export function validateA10CandidateBuild(value, qualifiedTree) {
  if (value?.schemaVersion !== 'stacks-attacknet-a10-candidate-build/v1'
    || value.qualifiedTree !== qualifiedTree || !Number.isFinite(Date.parse(value.capturedAt ?? ''))
    || value.build?.schemaVersion !== 'stacks-attacknet-local-build/v1'
    || value.install?.schemaVersion !== 'stacks-attacknet-local-install/v1') {
    fail('A10 candidate build does not pin the qualified tree');
  }
  const built = indexedImages(value.build.images, A10_BUILD_PURPOSES, 'candidate build');
  const installed = indexedImages(value.install.images, A10_INSTALLED_PURPOSES, 'candidate install');
  for (const purpose of A10_INSTALLED_PURPOSES) {
    if (installed.get(purpose).id !== built.get(purpose).id) fail(`candidate install did not use built ${purpose}`);
  }
  const installLoad = validateKindLoad(value.install.kindImageLoad,
  ...
  for (const purpose of ['stacks-core', 'stacker']) {
    ...
    if (actor.ref !== built.get(purpose).ref || actor.immutableID !== built.get(purpose).id) fail(`candidate receipt does not bind ${purpose}`);
    actorRefs.add(actor.ref);
  }
  const actorLoad = validateKindLoad(value.actorImageLoad, actorRefs, installLoad.nodes);
  if (value.runOperatorImageID !== built.get('run-operator').id) fail('candidate receipt has an inconsistent run-operator image ID');
