...
const sourceEnvironment = 'ATTACKNET_A12_MATERIALIZED_SOURCE';
...
/** Materialize one immutable A12 qualified tree in an isolated shared clone. */
export function materializeQualifiedTree(repositoryRoot, qualifiedTree) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'attacknet-a12-source-'));
  ...
  try {
    ...
    return {
      sourceRoot, environment: {...environment, [sourceEnvironment]: sourceRoot},
...
/** Re-execute an A12 qualification entrypoint from its immutable source tree. */
export function runMaterializedEntrypoint({repositoryRoot, qualifiedTree, script, arguments_}) {
  ...
  try {
    execute(process.execPath, [join(materialized.sourceRoot, script), ...arguments_], {
      cwd: materialized.sourceRoot, env: materialized.environment, stdio: 'inherit',
    });
  } finally {
...
/** Report whether this process already runs from the materialized A12 tree. */
export function isMaterializedSource(repositoryRoot) {
  try {
    return Boolean(process.env[sourceEnvironment])
      && realpathSync(process.env[sourceEnvironment]) === realpathSync(repositoryRoot);
  } catch {
    return false;
  }
}
