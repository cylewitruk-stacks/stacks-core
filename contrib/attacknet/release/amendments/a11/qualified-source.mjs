...
const sourceEnvironment = 'ATTACKNET_A11_MATERIALIZED_SOURCE';
...
/** Materialize one immutable A11 qualified tree in an isolated shared clone. */
export function materializeQualifiedTree(repositoryRoot, qualifiedTree) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'attacknet-a11-source-'));
...
/** Re-execute an A11 qualification entrypoint from its immutable source tree. */
...
/** Report whether this process already runs from the materialized A11 tree. */
