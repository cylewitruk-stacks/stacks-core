  ...
  outputs =
    ...
    flake-utils.lib.eachSystem (
      ...
      let
        ...
        workspaceCrates = lib.concatMap (
          crate:
          if lib.hasSuffix "/*" crate then
            let
              directory = lib.removeSuffix "/*" crate;
              directoryPath = ../.. + "/${directory}";
              entries = builtins.readDir directoryPath;
              crateNames = builtins.attrNames (
                lib.filterAttrs (
                  name: _type: builtins.pathExists (directoryPath + "/${name}/Cargo.toml")
                ) entries
              );
            in
            map (name: directoryPath + "/${name}") crateNames
          else
            [ (../.. + "/${crate}") ]
        ) cargoWorkspace.members;
